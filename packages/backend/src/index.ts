...
import { getArrayExtentNames, getScalarBounds, type Param, type Type } from "@jplmm/ast";
...
function emitParamNormalizationPrelude(params: Param[]): string {
  return params
    .flatMap((param) => {
...
function emitScalarNormalizationOps(type: Type | undefined): string[] {
  if (!type) {
    return [];
  }
  if (type.tag === "int") {
...
function emitParamExtentPrelude(params: Param[]): string {
  const lines: string[] = [];
