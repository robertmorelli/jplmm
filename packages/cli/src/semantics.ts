...
export function buildSemanticsDebugData(
  ...
  backend: SemanticsBackendOrNull = null,
...
function buildLutImplementationFloor(
  ...
  const functions = [...implementations.entries()]
    .filter((entry): entry is [string, Extract<OptimizeResult["artifacts"]["implementations"] extends Map<string, infer T> ? T : never, { tag: "lut" }>] => entry[1].tag === "lut")
    .sort(([left], [right]) => left.localeCompare(right))
...
function serializeRangeAnalysis(result: RangeAnalysisResult): SerializedRangeAnalysis {
  return {
...
function serializePlainIrAnalysis(
  trace:
    | {
        hasRec: boolean;
