...
export function matchAitkenPass(program) {
    const matches = [];
...
function inferTargetParamIndex(fn, stateParamIndex) {
    ...
    const floatInvariants = fn.params
        ...
        .filter(({ param, idx }) => idx !== stateParamIndex &&
...
function tailArgIsInvariant(fn, index) {
...
function matchesRadTargetMinusRes(expr, targetParamName) {
    const stripped = stripNanToZero(expr);
    if (stripped.tag !== "binop" || stripped.op !== "-") {
        return false;
    }
    return ((stripped.left.tag === "var" &&
...
function matchesRadStateDelta(expr, stateParamName) {
    const stripped = stripNanToZero(expr);
    if (stripped.tag !== "binop" || stripped.op !== "-") {
        return false;
    }
...
//# sourceMappingURL=aitken.js.map