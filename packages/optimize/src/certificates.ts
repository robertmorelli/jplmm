...
export function validateOptimizeCertificates(result: OptimizeResult): OptimizeCertificateChecks {
  return {
    canonicalize: validateCanonicalizePassCertificate(
      result.stages.rawProgram,
      result.stages.canonical.program,
      result.certificates.canonicalize,
    ),
    rangeAnalysis: validateRangeAnalysisPassCertificate(
      result.stages.canonical.program,
      result.certificates.rangeAnalysis,
    ),
    guardElimination: validateGuardEliminationPassCertificate(
      result.stages.canonical.program,
      result.stages.guardElided.program,
      result.certificates.guardElimination,
    ),
...
export function validateCanonicalizePassCertificate(
  ...
  const statsMatch =
    canonicalCounts.totalDiv - rawCounts.totalDiv === stats.totalDivInserted &&
...
export function validateRangeAnalysisPassCertificate(
  program: IRProgram,
...
export function validateGuardEliminationPassCertificate(
  canonicalProgram: IRProgram,
  ...
  const removedMatch =
    canonicalCounts.nanToZero - guardCounts.nanToZero === certificate.removed.nanToZero &&
...
export function validateFinalIdentityPassCertificate(
...
export function validateClosedFormPassCertificate(
  program: IRProgram,
...
export function validateLutPassCertificate(
  certificate: OptimizeResult["certificates"]["lut"],
): OptimizeCertificateValidation {
