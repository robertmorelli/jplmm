...
import type {
  ...
  LinearSpeculationMatch,
  ResearchCandidate,
} from "./types";
import { stripNanToZero } from "./utils";

export function findLinearSpeculationCandidates(program: IRProgram): Map<string, ResearchCandidate[]> {
  const out = new Map<string, ResearchCandidate[]>();
...
export function matchLinearSpeculationPass(program: IRProgram): LinearSpeculationMatch[] {
  const matches: LinearSpeculationMatch[] = [];
  for (const fn of program.functions) {
    const match = matchLinearTailRec(fn);
    if (match) {
...
function matchLinearTailRec(
  fn: IRFunction,
): { implementation: LinearSpeculationImplementation; candidate: ResearchCandidate } | null {
  ...
  let varyingParamIndex: number | null = null;
  ...
  const recurrence = analyzeLinearRecurrence(
    tailRet.expr.args[varyingParamIndex]!,
    fn.params[varyingParamIndex]!.name,
  );
  if (!recurrence) {
    return null;
  }
...
function analyzeLinearRecurrence(
  expr: IRExpr,
  paramName: string,
): { fixedPoint: number; stride: number; direction: "up" | "down" } | null {
...
function matchSignedDelta(expr: IRExpr, paramName: string): number | null {
  const stripped = stripNanToZero(expr);
