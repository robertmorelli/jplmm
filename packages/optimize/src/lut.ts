...
import type { CardinalityInfo, OptimizeArtifacts, TabulatedFunction } from "./types";

export function tabulateLuts(
  program: IRProgram,
  artifacts: OptimizeArtifacts,
  threshold: number,
): TabulatedFunction[] {
  const out: TabulatedFunction[] = [];
...
function enumerateArgs(
  ...
  const table: number[] = [];
  const current = new Array<number>(cardinality.parameterRanges.length);

  const loop = (idx: number): void => {
