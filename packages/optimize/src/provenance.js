export function buildExprProvenance(input, output, stage = "canonicalize") {
...
export function serializeExprProvenance(provenance) {
    return {
        byOutputExprId: Object.fromEntries([...provenance.byOutputExprId.entries()]
...
function assignExprProvenance(expr, inputIds, inputTags, byOutputExprId, stage) {
...
function inferProvenanceRule(stage, expr, status, inputTag) {
    if (stage === "identity") {
...
function mapExprTagToRule(tag, prefix) {
    switch (tag) {
        case "total_div":
...
function collectExprIds(expr, ids, tags) {
    if (ids.has(expr.id)) {
        return;
    }
    ids.add(expr.id);
