...
function evalBinaryRange(op, left, right, resultType) {
...
function evalCallRange(name, args, resultType) {
...
function evalTotalDivRange(left, right, resultType) {
...
function evalTotalModRange(left, right, resultType) {
    if (includesZero(right)) {
        return { lo: 0, hi: 0 };
    }
...
function includesZero(range) {
    return range.lo <= 0 && range.hi >= 0;
}
function mulRange(a, b) {
...
function computeCardinality(ranges, types) {
    let out = 1;
