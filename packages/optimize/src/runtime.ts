...
type Frame = {
  ...
  currentRes: RuntimeValue | undefined;
  fuel: number | "inf" | null;
  aitkenHistory: number[];
  aitkenImpl: AitkenImplementation | undefined;
};
...
type RuntimeContext = {
  ...
  program: IRProgram;
  artifacts: OptimizeArtifacts | undefined;
  stats: ExecuteStats;
};
...
export function executeProgram(
  ...
  const ctx: RuntimeContext = {
    functions: new Map(program.functions.map((fn) => [fn.name, fn] as const)),
...
function executeFunction(
  ...
  while (true) {
    ...
    let pendingTailArgs: RuntimeValue[] | null = null;
...
function handleTailRec(
  expr: Extract<IRExpr, { tag: "rec" }>,
  frame: Frame,
  ctx: RuntimeContext,
  depth: number,
): { kind: "return"; value: RuntimeValue } | { kind: "tail"; nextArgs: RuntimeValue[] } {
...
function evalExpr(expr: IRExpr, frame: Frame, ctx: RuntimeContext, depth: number): RuntimeValue {
...
function evalUnary(op: string, operand: RuntimeValue, resultType: Type): RuntimeValue {
  if (op !== "-") {
...
function evalBinary(op: string, left: RuntimeValue, right: RuntimeValue, resultType: Type): RuntimeValue {
...
function totalDiv(left: RuntimeValue, right: RuntimeValue, resultType: Type): RuntimeValue {
...
function totalMod(left: RuntimeValue, right: RuntimeValue, resultType: Type): RuntimeValue {
...
function operandNeg(value: number, resultType: Type): RuntimeValue {
  if (resultType.tag === "int") {
    if (value === INT32_MIN) {
      return INT32_MAX;
    }
...
function materializeArrayValue(
  arrayType: Type,
  items: RuntimeValue[],
  structs: Map<string, IRStructDef>,
): RuntimeArrayValue {
...
function evaluateArrayExpr(
  ...
  let suffixDims: number[] | null = null;
...
function evaluateSumExpr(
  ...
  let total: RuntimeValue = resultType.tag === "float" ? 0 : 0;
...
function indexArrayValue(
  arrayValue: RuntimeArrayValue,
  indices: number[],
  resultType: Type,
  structs: Map<string, IRStructDef>,
): RuntimeValue {
...
function argsMatchCurrent(
  fn: IRFunction,
  nextArgs: RuntimeValue[],
  currentParams: RuntimeValue[],
  structs: Map<string, IRStructDef>,
): boolean {
...
function normalizeByType(value: RuntimeValue, type: Type | undefined, structs?: Map<string, IRStructDef>): RuntimeValue {
  if (!type) {
    return value;
  }
...
function normalizeScalarByType(value: number, type: Type | undefined): number {
  if (!type) {
    return value;
  }
  if (type.tag === "int") {
...
function assertNumber(value: RuntimeValue | undefined, context: string): number {
...
function asScalarArgs(args: RuntimeValue[]): number[] | null {
...
function strideOf(dims: number[], index: number): number {
...
function getInitialFuel(fn: IRFunction): number | "inf" | null {
...
function tryEvalLut(impl: Extract<FunctionImplementation, { tag: "lut" }>, args: number[]): number | null {
...
function evalClosedFormLinear(
  impl: Extract<FunctionImplementation, { tag: "closed_form_linear_countdown" }>,
  args: number[],
): number {
...
function tryApplyAitken(
  impl: AitkenImplementation,
  frame: Frame,
  nextArgs: RuntimeValue[],
  structs: Map<string, IRStructDef>,
): RuntimeValue[] | null {
...
function applyLinearSpeculation(
  impl: Extract<FunctionImplementation, { tag: "linear_speculation" }>,
  args: number[],
  fn: IRFunction,
): RuntimeValue[] {
...
function defaultValueForType(type: Type, structs: Map<string, IRStructDef>): RuntimeValue {
  if (type.tag === "float") {
    return 0;
  }
...
function createStats(): ExecuteStats {
  return {
    exprEvaluations: 0,
    functionCalls: 0,
    recCalls: 0,
...
function recordImplementationHit(stats: ExecuteStats, name: string): void {
