...
import { analyzeIrFunction, analyzeIrGlobals, buildIrCallSummaries, renderIrExpr, } from "./ir";
...
export function deserializeRangeAnalysis(result) {
    return {
        rangeMap: new Map(Object.entries(result.exprRanges)
            .map(([id, range]) => [Number(id), range])
            .sort(([left], [right]) => left - right)),
        cardinalityMap: new Map(Object.entries(result.cardinalities)
            .map(([fnName, info]) => [fnName, info])
            .sort(([left], [right]) => left.localeCompare(right))),
    };
}
export function serializeRangeFacts(program, analysis, exprIds) {
...
export function buildCanonicalRangeSoundnessEdgeRecord(program, analysis, exprIds, solverOptions, certificate = null) {
    ...
    const verifyExprIds = (owner, fn, relevantExprIds, exprSemantics, callSigs, comparisonEnv, vacuousDetail) => {
        ...
        for (const exprId of relevantExprIds) {
            ...
            const verdict = proveScalarRangeFact(fn ?? {
                name: owner,
                keyword: "fun",
                params: [],
                retType: { tag: "void" },
                body: [],
                id: -1,
            }, callSigs, normalizedValue.expr, exprRange, comparisonEnv, edgeSolverOptions);
            if (!verdict.ok) {
                failures.push({
    ...
    const functions = [...program.functions]
        ...
        const relevantExprIds = [...fnAnalysis.exprSemantics.keys()]
            ...
            .sort((left, right) => left - right);
        return verifyExprIds(fn.name, fn, relevantExprIds, fnAnalysis.exprSemantics, fnAnalysis.callSigs, comparisonEnv, "no canonical range facts were attached to this function");
    });
    const globalComparisonEnv = new Map();
    const globalExprIds = [...globalAnalysis.exprSemantics.keys()]
        .filter((exprId) => targetExprIds.has(exprId))
        .sort((left, right) => left - right);
    if (program.globals.length > 0 || globalExprIds.length > 0) {
        functions.push(verifyExprIds("<globals>", null, globalExprIds, globalAnalysis.exprSemantics, globalAnalysis.callSigs, globalComparisonEnv, "no canonical range facts were attached to globals"));
    }
...
function proveScalarRangeFact(fn, callSigs, expr, range, comparisonEnv, solverOptions) {
...
function deriveScalarInterval(expr, comparisonEnv) {
    switch (expr.tag) {
        case "int_lit":
        case "float_lit":
        ...
        case "sat_add":
            return clampIntInterval(deriveBinaryInterval("+", expr.left, expr.right, comparisonEnv, true));
        case "sat_sub":
            return clampIntInterval(deriveBinaryInterval("-", expr.left, expr.right, comparisonEnv, true));
        case "sat_mul":
            return clampIntInterval(deriveBinaryInterval("*", expr.left, expr.right, comparisonEnv, true));
...
function deriveBinaryInterval(op, leftExpr, rightExpr, comparisonEnv, integer) {
...
function deriveCallInterval(expr, comparisonEnv) {
...
function clampIntInterval(interval) {
    if (!interval) {
        return null;
    }
    return {
...
function intervalContainedBy(derived, expected) {
...
//# sourceMappingURL=compiler_ladder_ranges.js.map