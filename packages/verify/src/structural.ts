...
export function checkStructuralDecrease(
  params: Param[],
  radExpr: Expr,
  recArgs: Expr[],
): StructuralCheck {
...
function trackedParam(params: Param[], radExpr: Expr): { name: string; index: number; absolute: boolean } | null {
...
function isParamMinusConst(paramName: string, expr: Expr): boolean {
...
function isMaxZeroParamMinusConst(paramName: string, expr: Expr): boolean {
...
function isAbsOfParam(paramName: string, expr: Expr): boolean {
  return (
    expr.tag === "call" &&
    expr.name === "abs" &&
    expr.args.length === 1 &&
...
export function collectRecArgs(expr: Expr, out: Expr[][]): void {
  switch (expr.tag) {
    case "rec":
      out.push(expr.args);
...
export function findRadExprs(stmts: Stmt[]): Expr[] {
  return stmts.filter((stmt): stmt is Extract<Stmt, { tag: "rad" }> => stmt.tag === "rad").map((stmt) => stmt.expr);
}

export function hasRec(stmts: Stmt[]): boolean {
  const bucket: Expr[][] = [];
...
export function collectRecSites(stmts: Stmt[]): Expr[][] {
  const bucket: Expr[][] = [];
  for (const s of stmts) {
    if (s.tag === "let" || s.tag === "ret" || s.tag === "rad") {
      collectRecArgs(s.expr, bucket);
